...
const string programName = "    UW Webcam Scheibenwischer Control Software";
...
void PrintHeader()
{
    ...
    LogInfo("    UW Webcam Scheibenwischer Control Software");
...
void PrintUsage()
{
    ...
    cout << "  *1st input: select the displacement mode: 'ROT' to move full rotations or 'DEG' for angular displacement under 360 degrees " << endl;
    cout << "  *2nd input: Wipers displacement: if 'ROT' is selected, insert a value between -20 and 20 rotations. Else if 'DEG' is select, insert a value beteween -360 and 360 degrees  " << endl;
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 4)
    {
        ...
        {
            new_targetPosition = displacement * 231 * 6 * (100 / 25);
        ...
        {
            new_targetPosition = displacement * ((231 * 6 * 4) / 360);

            if (displacement < -360 || displacement > 360)
            {
                cout << "   Degree displacement out of range, please insert a number between -360 and 360" << endl;
                CloseDevice(pErrorCode);
            }
        }
